...
### 3. PDF Export (김민구)
* **기능**: PDF에 담을 정보(모델 3D 렌더링 이미지, 메모 내용, 퀴즈 결과, AI 어시스턴트와 대화 내역)를 사용자가 선택하여 PDF로 변환 후 미리보기 및 저장 가능
* **구현**: commonmark-java 라이브러리와 thymeleaf를 통해 마크다운 문법이 적용된 html 생성, 이후 html을 바이너리 데이터로 변환 후 프론트엔드측에 전달하여 pdf 미리보기 및 저장 기능 구현 
