...
### 3. PDF Export (김민구)
* **기능**: PDF에 담을 정보(3D 렌더링 이미지, 메모 내용, 퀴즈 결과, AI 어시스턴트 대화 내역)를 사용자가 선택하여 PDF로 변환 후 미리보기 및 저장 가능
* **구현**: commonmark-java 라이브러리와 thymeleaf를 사용해 마크다운 문법 적용 html 생성, 이후 html을 바이너리 데이터로 변환 후 프론트엔드측에 전달하여 pdf 미리보기 및 저장 기능 구현 

### 4. AI Assist (구동한)
* **기능**: 사용자가 텍스트/이미지(URL)와 대화 히스토리를 함께 보내면, 멀티턴 문맥을 반영해 한국어 답변을 생성
* **구현**: OpenAiClient가 API를 기본 호출하며, 모델/부품 메타데이터를 프롬프트에 정규화 반영하고 시스템 프롬프트(app.chat.*) 및 토큰으로 응답 품질·지연을 제어

### 5. RAG (구동한)
* **기능**: 사용자의 질문과 문맥에 맞는 문서 청크를 벡터 유사도 검색으로 찾아 근거 컨텍스트를 생성하고, 답변에 [S1], [S2] 형태의 출처(문서/페이지)를 붙여 신뢰 가능한 응답 제공
* **구현**: RagService가 질문 임베딩을 생성한 뒤 pgvector(embedding <=> query)로 top-k 검색을 수행하고, PDF 업로드→텍스트 추출→청킹→임베딩 저장 파이프라인을 구축
